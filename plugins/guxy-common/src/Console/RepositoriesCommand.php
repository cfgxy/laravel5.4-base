...
class RepositoriesCommand extends Command
{
     ...
     * @var string
     */
    protected $name = 'guxy:ide-helper:models';
...
\n\n";
        ...
        $this->addNamedMethods(Model::class, '');
        $output .= $this->createRepoPhpDocs(Model::class);
        ...
        foreach ($models as $name) {
            ...
            if (class_exists($name)) {
                try {
                    ...
                    foreach ($this->properties as $p => $props) {
                        $this->addFindByMethods($name, $p);
                    }

                    $namedQueries = ['query'];
                    if (class_exists($name . 'Repository')) {
                        $reflectionRepoClass = new \ReflectionClass($name . 'Repository');
                        $methods = $reflectionRepoClass->getMethods(\ReflectionMethod::IS_PROTECTED);

                        $methods = collect($methods)->map(function(\ReflectionMethod $o) {
                            return $o->name;
                        })->all();

                        $methods = preg_grep('@^query\w+$@', $methods);
                        $namedQueries = array_merge($namedQueries, $methods);

                        $namedQueries = array_values(array_unique($namedQueries));
                    }

                    foreach ($namedQueries as $method) {
                        if (preg_match('@^queryBy(\w+)$@', $method, $m)) {
                            $this->addFindByMethods($name, Str::snake($m[1]));
                        } elseif (preg_match('@^query(\w*)$@', $method, $m)) {
                            $this->addNamedMethods($name, Str::snake($m[1]));
                        }
                    }

                    $this->repoMethods['retrieveByPK'] = [
                        'type'          =>  "\\{$name}",
                        'arguments'     => ['$id', '$limiters = []', '$options = []']
                    ];
                    $this->repoMethods['retrieveByPKs'] = [
                        'type'          =>  "\Illuminate\Database\Eloquent\Collection|\\{$name}[]",
                        'arguments'     => ['$ids', '$limiters = []', '$options = []']
                    ];

                    $this->getPropertiesFromMethods($model);
                    $output .= $this->createPhpDocs($name);

                    $output .= $this->createRepoPhpDocs($name);
                    $ignore[] = $name;
                } catch (\Exception $e) {
    ...
    protected function addNamedMethods($name, $p)
    {
        if (array_key_exists(Str::camel("get_{$p}Pager"), $this->repoMethods)) {
            return;
        }

        $arguments = ['$limiters = []', '$options = []'];
        $this->repoMethods[Str::camel("get_{$p}Pager")] = [
            'type'          =>  '\Illuminate\Contracts\Pagination\LengthAwarePaginator',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("get_{$p}Query")] = [
            'type'          =>  '\Illuminate\Database\Query\Builder',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("get_{$p}QueryBuilder")] = [
            'type'          =>  '\Illuminate\Database\Eloquent\Builder',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("get_{$p}SQL")] = [
            'type'          =>  'string',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("findOne_{$p}")] = [
            'type'          =>  '\\' . $name,
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("find_{$p}")] = [
            'type'          =>  "\Illuminate\Database\Eloquent\Collection|\\{$name}[]",
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("count_{$p}")] = [
            'type'          =>  "int",
            'arguments'     => $arguments
        ];
    }
    ...
    protected function addFindByMethods($name, $p)
    {
        if (array_key_exists(Str::camel("getPagerBy_{$p}"), $this->repoMethods)) {
            return;
        }

        $arguments = ["$$p", '$limiters = []', '$options = []'];
        $this->repoMethods[Str::camel("getPagerBy_{$p}")] = [
            'type'          =>  '\Illuminate\Contracts\Pagination\LengthAwarePaginator',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("getQueryBy_{$p}")] = [
            'type'          =>  '\Illuminate\Database\Query\Builder',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("getQueryBuilderBy_{$p}")] = [
            'type'          =>  '\Illuminate\Database\Eloquent\Builder',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("getSQLBy_{$p}")] = [
            'type'          =>  'string',
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("findOneBy_{$p}")] = [
            'type'          =>  '\\' . $name,
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("findBy_{$p}")] = [
            'type'          =>  "\Illuminate\Database\Eloquent\Collection|\\{$name}[]",
            'arguments'     => $arguments
        ];
        $this->repoMethods[Str::camel("countBy_{$p}")] = [
            'type'          =>  "int",
            'arguments'     => $arguments
        ];
    }
     ...
     * @return string
     */
    protected function createRepoPhpDocs($class)
    {
        ...
        if ($class === Model::class) {
            $repoClass = 'DefaultRepository';
            $repoNamespace = 'Guxy\Common\Database';
        } else {
        ...
        foreach ($this->repoMethods as $name => $method) {
        ...
        $output = "namespace {$repoNamespace}{\n{$docComment}\n\t abstract class {$repoClass} extends \Guxy\Common\Database\Repository {}\n}\n\n";
        return $output;
    }
    ...
    protected function createPhpDocs($class)
    {
        ...
        if (method_exists($class, 'repository')) {
            $this->methods['repository'] = [
